...
; (function(root, factory) {
    if (typeof module === 'object' && module.exports) {
        module.exports = factory() // CommonJS导出
    } else {
        root.qone = factory() // 全局导出
    }
}(this, function() {
    var WHITESPACE_CHARS = array2hash(characters(' \r\t\u200b')) // 所有空格字符
    var OPERATOR_CHARS = array2hash(characters('+-*&%=<>!?|~^')) // 操作符
    var PUNC_CHARS = array2hash(characters('[]{}(),;:')) // 标点符号
    var PUNC_BEFORE_EXPRESSION = array2hash(characters('[{}(,.;:')) // 标点前符号
    ...
    var KEYWORD = array2hash(['from', 'in', 'where', 'select', 'orderby', 'desc', 'asc', 'groupby', 'limit']) // 关键词
    var UNICODE = {
    ...
    */
    ...
    Lexer.prototype = {
        ...
        // 开始方法
        start: function() {
            var t = this.scan() // 获取token 
            if (t !== undefined) { 
                this.list.push(t) // 收集token 
                this.start() // 重新开始
            }
        }
    }
